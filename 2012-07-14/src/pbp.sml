  ...
  sig
	type prCnstr 
      ...
      val add_pbp_rule : string -> 
           (string option -> prCnstr -> int list -> 
...
in 
	...
	type prCnstr = LegoFormat.prCnstr
	val ffp = LegoFormat.ffp_pbp

	val initNameLoc = Namespace.initNameLoc 
	val mkNameLoc   = Namespace.mkNameLoc 
	val NextTac     = Namespace.Next 

	fun getCurrentGoal () =
	    case Namespace.getCurrentQuest() of
		((Namespace.Unknown(goals)::_)) =>
		let
		   val (goal::_) = Namespace.unGoals goals 
		   val (exvar,formula) = Namespace.unGoal goal 
		in 
		   (exvar,ffp formula)
		end
        ...
        fun getAssumption s = 
	    let
		val T = Engine.RefTyp_s s (* (_,T) = Concrete.fEvalNam s *)
	    in
		ffp T
	    end 
end 
...
functor FunPbp (structure LegoFormat:LEGOFORMAT 
	        structure PbpInput:PBPINPUT
		sharing type PbpInput.prCnstr = LegoFormat.prCnstr
		)  : PBP  =
struct

	type prCnstr = LegoFormat.prCnstr (* we need to do matching on it *)
...
local
	...
	val HYPNAME = Utils.StringUtil.HYPNAME 

	val message = Printing.message 
in
	val std_out = TextIO.stdOut (* does this need rebinding? *)

        fun output_string_list l = 
                message (List.foldr (fn (a,b)  => a^"; "^b) "" l);

	val format_commands = ListFormat.fmt {init="",sep=" ",final="",fmt=fn a=>a}
	 ...
	 fun pbp_forall
		 ...
		 let 
		     val (a::id_list, A, B) = LegoFormat.getPrPi pr_formula
		 in 
		  (case path 
		   of [1] => pbp_forall hyp_name pr_formula [1,length(a::id_list)] continuation
		    | 2::path_tail =>
			let
		    ...
		    | _ => 
			   ...
			   SOME hyp_name =>
			   	(case path of
				    ...
				    | _ => let
					    val new_name1 = PbpInput.mkNameLoc HYPNAME
				...
				(** Notice that we ignore the substitution B[?j/x] **)
						...
						case id_list of
						     [] => B
						   | _  => LegoFormat.mkPrPi (id_list, A, B)

					    val path =
						case path of
						     1::i::path_tail => 1::(i-1)::path_tail
						   | 3::path_tail => 
						     (case id_list of [] => path_tail 
						     	   	    | _ => path)
			 ...
			 * uniformly, but for efficiency reasons, we employ pbp_forall 
			 * rather than the continuation whenever possible. *)
						case id_list of
				    ...
				    | NONE =>
					  (let
					       val used_names = ref []
					       fun command i = (List.app (fn x =>
									 used_names := (PbpInput.mkNameLoc x)::(!used_names))
								
								(List.rev (List.take (a::id_list,i)));
...
fun pbp_let
    ...
    (pr_formula:prCnstr)
    (path: int list)
    (continuation_function: string option  -> LegoFormat.prCnstr -> int list -> string list) =
    let 
	val (id_list, _, continuation_formula) = LegoFormat.getPrLet pr_formula
    in 
	(case hyp_name of 
	   SOME hyp_name => 
	     (case path of 
...
(* ************************************************************** *
fun pbp_impr2 (SOME H) (LegoFormat.PrBind([], (Pi,Vis), _, B))
...
fun pbp_imply
    ...
    (continuation_function: string option 
     ...
     let 
     	 ...
     	 val (type_formula, continuation_formula) = LegoFormat.getPrArr pr_formula
...
fun pbp_string_nl s = (Annotate.pbpAnnotate s)^"\n" (* *** new *** *)
val pbp_result     = pbp_string_nl     "Pbp result"
val pbp_end_result = pbp_string_nl "End Pbp result"
...
fun pbptop_default goal_no path =
	...
	val string_list = (if exvar=goal_no
			       then (fn id=>id)
			   else fn res => ("Next " ^Int.toString goal_no)::res)
	    (pbp NONE goal path)
    in
	(TextIO.output (std_out, pbp_result); (* *** *)
	 output_string_list string_list;
	 TextIO.output (std_out, pbp_end_result))(* *** *)
    end
